# Code just does not converge properly, there is something wrong.
# One should now restart from the original script: Stokes2D_vep_reg_ctau.jl
# and make sure we get same results using GeoParams
# Initialisation
...
@views function UpdateStressNative_VEP_invariants!(ηe_c, ηe_v, ηve_c, ηve_v, τxx, τyy, τxy, ε̇xx, ε̇yy, ε̇xy, τxx0, τyy0, τxy0,
    ε̇ii, τii, τii0, ε̇xy_c, τxy_c,ε̇xy2_c, τxy2_c, F, τ_y, Pt, sinϕ, η_reg, Pla, λ, λ_phys, λ_rel, ε̇ii_pl, Fchk, ηc, ηv)
    ...
    Pla    .= 0.0
    Pla    .= F .> 0.0
    λ_phys .= Pla.*F./(ηve_c .+ η_reg)
    λ      .= λ_rel*λ_phys .+ (1.0.-λ_rel).*λ
...
@views function Stokes2D_VE_inclusion(UseGeoParams, doPlot = false)
    ...
    for it = 1:nt
        ...
        while (err>ε && iter<=iterMax)
            ...
            # κΔτp .= cfl .* ηc .* Δx ./ Lx  
            # PT timestep
            Δτvx[2:end-1,:]   .= min(Δx,Δy)^2.0./av_xa(ηc)./4.1./Vsc
            Δτvy[:,2:end-1]   .= min(Δx,Δy)^2.0./av_ya(ηc)./4.1./Vsc
            κΔτp   .= 4.1.*ηc./max(ncx,ncy)./Ptsc
            # Calculate rate update --------------------------------------
            # dVxdτ          .= (1-ρ) .* dVxdτ .+ Rx
            # dVydτ          .= (1-ρ) .* dVydτ .+ Ry
            dVxdτ          .= (1-Vdmp/ncx) .* dVxdτ .+ Rx
            dVydτ          .= (1-Vdmp/ncy) .* dVydτ .+ Ry
            dPdτ           .= Rp
            # Update velocity and pressure -------------------------------
            # Vx[:,2:end-1]  .+= Δτvx ./ ρ .* dVxdτ
            # Vy[2:end-1,:]  .+= Δτvy ./ ρ .* dVydτ
            Vx[:,2:end-1]  .+= Δτvx .* dVxdτ
            Vy[2:end-1,:]  .+= Δτvy  .* dVydτ
            Pt             .+= κΔτp .* dPdτ
            # convergence check
       ...
       # error("stop here")
        ...
        if doPlot
            ...
            p2 = heatmap(xc, yc, ηc' , aspect_ratio=1, xlims=(Δx/2, Lx-Δx/2), ylims=(0, Ly), c=:inferno, title="η_vep")
            p3 = heatmap(xc, yc, τii' , aspect_ratio=1, xlims=(Δx/2, Lx-Δx/2), ylims=(0, Ly), c=:inferno, title="τii")
