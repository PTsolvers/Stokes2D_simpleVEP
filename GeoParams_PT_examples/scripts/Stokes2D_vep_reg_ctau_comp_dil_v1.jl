# Initialisation
# Add bulk elasticity in basic shear banding script
...
@views function Stokes2D_vep()
    ...
    for it = 1:nt
        ...
        Pt_o.=Pt; Txx_o.=Txx; Tyy_o.=Tyy; Txy_o.=av(Txyv); Txyv_o.=Txyv; λ.=0.0
        local itg
        while (err>ε && iter<=iterMax)
            ...
            # Assume this comes from GP (sets local P residual to 0.0) 
            Pt     .= Pt_o - Κ*Δt*∇V 
            # yield function
            F      .= Tii .- τ_y .- Pt_it.*sinϕ
            Pla    .= 0.0
            Pla    .= F .> 0.0
            λ      .= Pla.*F./(η_ve .+ η_reg .+ Κ*Δt*sinψ*sinϕ)
            dQdTxx .= 0.5.*Txx./Tii
            dQdTyy .= 0.5.*Tyy./Tii
            dQdTxy .=      Txy./Tii
            # plastic corrections
            ...
            ∇Vp    .= λ1*sinψ
            Tii    .= Tii  .- λ1.*η_reg
            Fchk   .= Tii  .- τ_y .- (Pt_it .+ Κ.*Δt.*∇Vp).*sinϕ 
            η_vep  .= Tii./2.0./Eii
            ...
            # You still need to update pressure iteratively
            # one possibility is to iterate pressure until it is equal to GP value
            RPt    .= (Pt .- Pt_it)
            Pt_it  .= Pt_it .+ dtPt.*RPt
            # Residuals
            Rx     .= .-diff(Pt_it .+ Κ.*Δt.*∇Vp, dims=1)./dx .+ diff(Txx, dims=1)./dx .+ diff(Txyv[2:end-1,:], dims=2)./dy
            Ry     .= .-diff(Pt_it .+ Κ.*Δt.*∇Vp, dims=2)./dy .+ diff(Tyy, dims=2)./dy .+ diff(Txyv[:,2:end-1], dims=1)./dx .+ av_ya(Rog)
            # Damping of momentum residuals
            ...
            # convergence check
            if mod(iter, nout)==0
                norm_Rx = norm(Rx)/length(Rx); norm_Ry = norm(Ry)/length(Ry); norm_RPt = norm(RPt)/length(RPt)
                err = maximum([norm_Rx, norm_Ry, norm_RPt])
                push!(err_evo1, err); push!(err_evo2, itg)
                @printf("it = %d, iter = %d, err = %1.2e norm[Rx=%1.2e, Ry=%1.2e, RPt=%1.2e] (Fchk=%1.2e) \n", it, itg, err, norm_Rx, norm_Ry, norm_RPt, maximum(Fchk))
            end
            iter+=1; itg=iter
        end
        t = t + dt
