...
@views function Stokes2D_vep()
    ...
    for it = 1:nt
        ...
        while (err>ε && iter<=iterMax)
            ...
            Tii    .= sqrt.(0.5*(Txx.^2 .+ Tyy.^2) .+ Txy.^2)
            Fchk   .= Tii .- τ_y .- Pt_it.*sinϕ .- λ.*η_reg
            η_vep  .= Tii./2.0./Eii
            ...
            # Residuals
            Rx     .= .-diff(Pt_it, dims=1)./dx .+ diff(Txx, dims=1)./dx .+ diff(Txyv[2:end-1,:], dims=2)./dy
            Ry     .= .-diff(Pt_it, dims=2)./dy .+ diff(Tyy, dims=2)./dy .+ diff(Txyv[:,2:end-1], dims=1)./dx .+ av_ya(Rog)
