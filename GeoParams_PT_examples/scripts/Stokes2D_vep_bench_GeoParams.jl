...
# end
@views function UpdateStressGeoParams!( τxx, τyy, τxy, ε̇xx, ε̇yy, ε̇xy, τxx0, τyy0, τxy0, MatParam, dt )
    ...
    for i in eachindex(τxx)
        ...
        args   = (; τII_old = τii0c[i], dt=dt, P=0.0)
    ...
    for i in eachindex(τxy)
        ...
        η_eff  = computeViscosity_εII(v, ε̇iiv[i], args, P=0.0)
...
@views function Stokes2D_ve()
    ...
    MatParam = (SetMaterialParams(Name="Matrix", Phase=1,
                CompositeRheology = CompositeRheology(
                    ConstantElasticity(G=G),
                    LinearViscous(η=η0),
                    DruckerPrager(C=1.5, ϕ=0))),)
    ...
    for it = 1:nt
        ...
        while (err>ε && iter<=iterMax)
            ...
            # stresses
            UpdateStressGeoParams!( τxx, τyy, τxy, ε̇xx, ε̇yy, ε̇xy, τxx0, τyy0, τxy0, MatParam, dt )
