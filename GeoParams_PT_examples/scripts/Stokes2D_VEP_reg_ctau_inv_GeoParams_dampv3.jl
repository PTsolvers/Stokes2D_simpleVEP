...
@views avx(A) = amean.(A[1:(end - 1), :], A[2:end, :])
@views avy(A) = amean.(A[:, 1:(end - 1)], A[:, 2:end])
@views function avxy(A)
    return amean4.(
        A[1:(end - 1), 1:(end - 1)],
        A[2:end, 1:(end - 1)],
        A[1:(end - 1), 2:end],
...
function update_stress_GP2!(
    ...
    Threads.@threads for j in axes(Exx, 2)
        @inbounds for i in axes(Exx, 1)
            ...
            phases_v = (Phasev[i, j], Phasev[i + 1, j], Phasev[i, j + 1], Phasev[i + 1, j + 1]) # gather vertices around ij center
    ...
    return Txyv[2:(end - 1), 2:(end - 1)] .= ameanxy(Txy)
end
...
@views function update_stresses!(
    ...
    Exyv[2:(end - 1), 2:(end - 1)] .=
        0.5 *
        (diff(Vx[2:(end - 1), :]; dims=2) ./ dy .+ diff(Vy[:, 2:(end - 1)]; dims=1) ./ dx)
    Exy .= ameanxy(Exyv)
    # visco-elastic strain rates
    Exx_ve .= Exx .+ 0.5 .* Txx_o ./ (G .* dt)
    Eyy_ve .= Eyy .+ 0.5 .* Tyy_o ./ (G .* dt)
    Exy_ve .= Exy .+ 0.5 .* Txy_o ./ (G .* dt)
    Exyv_ve = Exyv .+ 0.5 .* Txyv_o ./ (Gv .* dt)
    # EII_ve .= sqrt.(0.5.*(Exx_ve.^2 .+ Eyy_ve.^2) .+ Exy_ve.^2)
    EII_ve .= sqrt.(0.5 .* (Exx_ve .^ 2 .+ Eyy_ve .^ 2) .+ ameanxy_pow2(Exyv_ve))
    # stress increments
    ...
    Txyv[2:(end - 1), 2:(end - 1)] .= ameanxy(Txy)
    # TII   .= sqrt.(0.5.*((Txx).^2 .+ (Tyy).^2) .+ (Txy).^2)
    TII .= sqrt.(0.5 .* ((Txx) .^ 2 .+ (Tyy) .^ 2) .+ ameanxy_pow2(Txyv))
    η_vep .= TII ./ 2.0 ./ EII_ve
    return nothing
end

@views function update_P_strain!(
    Pr,
    Pr_old,
    dPr,
    K,
...
@views function update_velocities!(
    Vx, Vy, Pr_c, Txx, Tyy, Txyv, ητ, ρgx, ρgy, vdτ, lτ, re_mech, dx, dy
)
    ηdτ = vdτ * lτ / re_mech
    Vx[2:(end - 1), :] .+=
        (
            diff(.-Pr_c .+ Txx; dims=1) ./ dx .+ diff(Txyv[2:(end - 1), :]; dims=2) ./ dy .-
            ρgx
        ) .* ηdτ ./ avx(ητ)
    Vy[:, 2:(end - 1)] .+=
        (
            diff(.-Pr_c .+ Tyy; dims=2) ./ dy .+ diff(Txyv[:, 2:(end - 1)]; dims=1) ./ dx .-
            ρgy
        ) .* ηdτ ./ avy(ητ)
    return nothing
end
...
@views function compute_residuals!(r_Vx, r_Vy, Pr_c, Txx, Tyy, Txyv, ρgx, ρgy, dx, dy)
    r_Vx .=
        diff(.-Pr_c[:, 2:(end - 1)] .+ Txx[:, 2:(end - 1)]; dims=1) ./ dx .+
        diff(Txyv[2:(end - 1), 2:(end - 1)]; dims=2) ./ dy .- ρgx[:, 2:(end - 1)]
    r_Vy .=
        diff(.-Pr_c[2:(end - 1), :] .+ Tyy[2:(end - 1), :]; dims=2) ./ dy .+
        diff(Txyv[2:(end - 1), 2:(end - 1)]; dims=1) ./ dx .- ρgy[2:(end - 1), :]
    return nothing
end
...
function main(UseGP, nt)
    ...
    for it in 1:nt
        @printf("it=%d\n", it)
        update_old!(Txx_o, Tyy_o, Txy_o, Txyv_o, Pt_old, Txx, Tyy, Txy, Txyv, Pt)
        errs = 2.0 .* ϵtol
        iter = 1
        # resize!(iter_evo, 0)
        ...
        while any(errs .>= ϵtol) && iter <= maxiter
            ...
            update_velocities!(
                Vx, Vy, Pt, Txx, Tyy, Txyv, ητ, ρgx, ρgy, vdτ, lτ, re_mech, dx, dy
            )
