...
@views function Stokes2D_vep()
    ...
    CFL     = 0.06/sqrt(2)        # CFL number # DEBUG was 0.9
    # Preprocessing
    dx, dy  = Lx/nx, Ly/ny
    max_lxy = max(Lx,Ly)
    Vpdτ    = min(dx,dy)*CFL
    ...
    for it = 1:nt
        ...
        while (err>ε && iter<=iterMax)
            ...
            # η_veτ[2:end-1,2:end-1] .= maxloc(η_ve); η_veτ[1,:] .= η_veτ[2,:]; η_veτ[end,:] .= η_veτ[end-1,:]; η_veτ[:,1] .= η_veτ[:,2]; η_veτ[:,end] .= η_veτ[:,end-1]
            # dτ_Rho .= Vpdτ .* max_lxy ./ Re ./ η_veτ  # could be η_veτ here with maxloc...
            
            η_vepτ[2:end-1,2:end-1] .= maxloc(η_vep); η_vepτ[1,:] .= η_vepτ[2,:]; η_vepτ[end,:] .= η_vepτ[end-1,:]; η_vepτ[:,1] .= η_vepτ[:,2]; η_vepτ[:,end] .= η_vepτ[:,end-1]
            dτ_Rho .= Vpdτ .* max_lxy ./ Re ./ η_vepτ # could be η_veτ here with maxloc...
            ...
            # Tyy    .= 2.0.*η_ve.*Eyy1
            # Txy    .= 2.0.*η_ve.*Exy1
            Txx    .= (Txx .+ 2.0 .* Gdτ .* Exx1) ./ (Gdτ ./ η_ve .+ 1.0)
            Tyy    .= (Tyy .+ 2.0 .* Gdτ .* Eyy1) ./ (Gdτ ./ η_ve .+ 1.0)
            Txy    .= (Txy .+ 2.0 .* Gdτ .* Exy1) ./ (Gdτ ./ η_ve .+ 1.0)
            ...
            Txx    .= (Txx .+ 2.0 .* Gdτ .* (Exx1 .-        λ .* dQdTxx)) ./ (Gdτ ./ η_ve .+ 1.0)
            Tyy    .= (Tyy .+ 2.0 .* Gdτ .* (Eyy1 .-        λ .* dQdTyy)) ./ (Gdτ ./ η_ve .+ 1.0)
            Txy    .= (Txy .+ 2.0 .* Gdτ .* (Exy1 .- 0.5 .* λ .* dQdTxy)) ./ (Gdτ ./ η_ve .+ 1.0)
            Tii    .= sqrt.(0.5*(Txx.^2 .+ Tyy.^2) .+ Txy.^2)
            ...
            # velocities
            dVx    .= av_xa(dτ_Rho) .* (.-diff(Pt, dims=1)./dx .+ diff(Txx, dims=1)./dx .+ diff(Txyv[2:end-1,:], dims=2)./dy)
            dVy    .= av_ya(dτ_Rho) .* (.-diff(Pt, dims=2)./dy .+ diff(Tyy, dims=2)./dy .+ diff(Txyv[:,2:end-1], dims=1)./dx .+ av_ya(Rog))
